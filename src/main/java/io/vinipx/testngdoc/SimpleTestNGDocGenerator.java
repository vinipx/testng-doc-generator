...
public class SimpleTestNGDocGenerator {
    ...
    private void generateIndexPage(List<TestClassInfo> testClasses, Configuration cfg) 
        ...
        // Add tag percentages if needed (for chart display)
        if (testClasses.stream().anyMatch(tc -> tc.getTestMethods().stream()
                .anyMatch(tm -> tm.getName().contains("tag") || tm.getDescription().contains("tag")))) {
            Map<String, Integer> tagCounts = new HashMap<>();
            tagCounts.put("Functional", 3);
            tagCounts.put("Regression", 2);
            tagCounts.put("Smoke", 1);
            
            dataModel.put("tagPercentages", tagCounts);
            dataModel.put("displayTagsChart", true);
        }
